...
const app = {
     ...
     */
    showClearAllConfirm() {
        const count = this.savedRings.length;
        if (count === 0) {
            this.showSaveSuccess('Collection is already empty');
            return;
        }

        const modal = document.getElementById('clearAllModal');
        const countEl = document.getElementById('clearAllCount');
        const input = document.getElementById('clearAllInput');
    ...
    /**
     * Hide the clear all confirmation modal
     */
    hideClearAllConfirm() {
        const modal = document.getElementById('clearAllModal');
        modal.classList.remove('open');
    ...
    /**
     * Validate clear all input and enable/disable confirm button
     */
    validateClearAllInput() {
        const input = document.getElementById('clearAllInput');
        const confirmBtn = document.getElementById('clearAllConfirmBtn');
        confirmBtn.disabled = input.value.toUpperCase() !== 'CLEAR';
    },

    /**
     * Clear all rings from collection (after confirmation)
     */
    async clearAllRings() {
        const input = document.getElementById('clearAllInput');
        ...
        const progressEl = document.getElementById('clearAllProgress');
        if (progressEl) {
            progressEl.style.display = 'block';
            progressEl.textContent = 'Clearing...';
        }

        try {
            const result = await FirebaseClient.clearAllRings((deleted, total) => {
                if (progressEl) {
                    progressEl.textContent = `Clearing ${deleted}/${total}...`;
                }
            });

            if (result.success || result.deleted > 0) {
                await this.loadCollection();
                this.showSaveSuccess(`Cleared ${result.deleted} ring${result.deleted !== 1 ? 's' : ''}`);
            } else if (result.deleted === 0 && !result.error) {
                this.showSaveSuccess('Collection was already empty');
            } else {
                throw new Error(result.error || 'Failed to clear collection');
            }
        } catch (error) {
            console.error('Clear all failed:', error);
            alert('Could not clear collection. Please try again.');
        } finally {
            this.showLoading(false);
            if (progressEl) {
